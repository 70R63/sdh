				...
				<ycommerce:testId code="RelationshipHistoryItem_showRelationships">
							...
							<%-- <hr align="left" width="50%" /> --%>
									...
									<c:if test="${relationship.status eq 'ACTIVE' && relationship.permissionsGivenOnRequestActiveCount eq 0 && relationship.permissionsGivenOnOwnActiveCount eq 0 
										...
										<spring:theme code="text.permission.both.user.basic" arguments="${otherUser.firstName}" htmlEscape="false" var="basicPermissions"/>
										${ycommerce:sanitizeHTML(basicPermissions)}<br/>
									</c:if>
									...
									<c:if test="${relationship.status eq 'ACTIVE' && relationship.permissionsGivenOnRequestPendingCount > 0}">
										<spring:theme code="text.permission.other.add.pending" arguments="${otherUser.firstName}" htmlEscape="false" var="permissionOtherAddPending"/>
										${ycommerce:sanitizeHTML(permissionOtherAddPending)}
									</c:if>
									</c:otherwise>
									</c:choose>
									...
									<c:if test="${relationship.status eq 'ACTIVE' && relationship.permissionsGivenOnOwnPendingCount > 0}">
										<spring:theme code="text.permission.current.add.pending" arguments="${otherUser.firstName}" htmlEscape="false" var="permissionCurrentAddPending"/>
										${ycommerce:sanitizeHTML(permissionCurrentAddPending)}
									</c:if>
									...
									<c:if test="${relationship.status eq 'PENDING' && relationship.sourceUser.uid eq customer.uid && empty relationship.targetUser}">
										<c:choose>
											...
											<c:otherwise>
												<spring:theme code="text.permission.current.initiate.nonuser.receiver.other" arguments="${relationship.targetEmail}, ${customer.uid}" htmlEscape="false" var="permissionCurrentInitiateNonUserReceiver"/>
									...
									<c:if test="${relationship.status eq 'PENDING' && relationship.sourceUser.uid eq customer.uid && not empty relationship.targetUser}">
										<c:choose>
											...
											<c:otherwise>
											<spring:theme code="text.permission.current.initiate.user.receiver.other" arguments="${relationship.firstName}, ${customer.uid}" htmlEscape="false" var="permissionCurrentInitiateUserReceiver"/>
