	...
	<div class="container">
		...
		<div id="item_container_holder">
			...
			<div class="item_container">
				<div>
					<h3>
						<span tabindex="0"><spring:theme code="checkout.multi.hostedOrderPostPage.header.wait" /></span>
			...
			<c:if test="${hopDebugMode}">
				<div class="item_container">
					<div id="debugWelcome">
						<h3>
							<span tabindex="0"><spring:theme code="checkout.multi.hostedOrderPostPage.header.debug" /></span>
						</h3>
						<hr />
					</div>
				</div>
							...
							<c:forEach items="${hostedOrderPageData.parameters}" var="entry" varStatus="status">
								<c:choose>
									<c:when test="${hopDebugMode}">
										<dt>
											<label for="${entry.key}" class="required"><c:out value="${entry.key}" /></label>
										</dt>
										<dd>
											<input type="text" id="${entry.key}" name="${entry.key}" value="${entry.value}" tabindex="${status.count + 1}" />
										</dd>
									</c:when>
									<c:otherwise>
