...
<ycommerce:testId code="facetNav_title_${facetData.name}">
	<div class="facet js-facet">
		<div class="facet__name js-facet-name">
			...
			<h3><span tabindex="0"><spring:theme code="search.nav.facetTitle" arguments="${facetData.name}"/></span></h3>
		</div>
		...
		<div class="facet__values js-facet-values js-facet-form">
					...
					<c:forEach items="${facetData.topValues}" var="facetValue">
						<li>
							<c:if test="${facetData.multiSelect}">
								<form action="#" method="get" tabindex="0" class="js-facet-form">
									<input type="hidden" name="q" value="${ycommerce:encodeHTML(facetValue.query.query.value)}"/>
									<input type="hidden" name="text" value="${ycommerce:encodeHTML(searchPageData.freeTextSearch)}"/>
									<label>
										<input class="facet__list__checkbox js-facet-checkbox sr-only" type="checkbox" ${facetValue.selected ? 'checked="checked"' : ''}/>
										<spring:theme code="facet.checkbox.checked.info" arguments="${facetValue.selected ? 'checked' : 'not checked'}" var="checkboxValue"/>
										<span class="facet__list__label">
											<span class="facet__list__mark" aria-label="${checkboxValue}"></span>
											<span class="facet__list__text">
												${ycommerce:encodeHTML(facetValue.name)}
							...
							<c:if test="${not facetData.multiSelect}">
								...
								<span class="facet__text">
									<a href="${facetValueQueryUrl}&amp;text=${ycommerce:encodeHTML(searchPageData.freeTextSearch)}">${ycommerce:encodeHTML(facetValue.name)}</a>&nbsp;
									<ycommerce:testId code="facetNav_count">
										<span class="facet__value__count"><spring:theme code="search.nav.facetValueCount" arguments="${facetValue.count}"/></span>
									</ycommerce:testId>
								</span>
				...
				<c:forEach items="${facetData.values}" var="facetValue">
					<li> 
						...
						<c:if test="${not facetData.multiSelect}">
							<c:url value="${facetValue.query.url}" var="facetValueQueryUrl"/>
							<span class="facet__text">
								<a href="${facetValueQueryUrl}">${ycommerce:encodeHTML(facetValue.name)}</a>
								<ycommerce:testId code="facetNav_count">
									<span class="facet__value__count"><spring:theme code="search.nav.facetValueCount" arguments="${facetValue.count}"/></span>
								</ycommerce:testId>
							</span>
			...
			<c:if test="${not empty facetData.topValues}">
				<span class="facet__value__more js-more-facet-values">
					<a href="#" class="js-more-facet-values-link" ><spring:theme code="search.nav.facetShowMore_${facetData.code}" /></a>
				</span>
				<span class="facet__value__less js-less-facet-values">
