...
public class DefaultSDHRopService implements SDHRopService
{
	 ...
	 */
	@Override
	public String rop(final RopRequest request)
	{

		final String urlString = configurationService.getConfiguration().getString("sdh.rop.url");
		final String user = configurationService.getConfiguration().getString("sdh.rop.user");
		final String password = configurationService.getConfiguration().getString("sdh.rop.password");

		if (StringUtils.isAnyBlank(urlString, user, password))
		{
			throw new RuntimeException("Error in ROP: Empty credentials");
		}
		...
		{
			LOG.error("There was an error in ROP: " + e.getMessage());
		}
