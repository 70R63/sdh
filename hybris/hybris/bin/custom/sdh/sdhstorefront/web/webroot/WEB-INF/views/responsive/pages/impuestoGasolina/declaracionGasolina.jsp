...
<script>
	function borrarMenu(bPredial, bVehicular, bIca, bPublicidadExt,
			bSobreGasolina, bDelineacionUrbana) {
		debugger;
		...
		for (submenu_index = 1; submenu_index <= 6; submenu_index++) {
			for (li_index = 7; li_index <= 19; li_index++) {
				...
				for (a_index = 0; a_index < a_array.length; a_index++) {
					...
					if (((bDelineacionUrbana != "X") && (itext
							.includes("delineacion-urbana")))
							|| ((bIca != "X") && (itext.includes("icareteica")))
							|| ((bPredial != "X") && (itext
									.includes("predialunificado")))
							|| ((bPublicidadExt != "X") && (itext
									.includes("publicidadexterior")))
							|| ((bVehicular != "X") && (itext
									.includes("sobrevehiculosautomotores")))
							|| ((bSobreGasolina != "X") && (itext
									.includes("sobretasa-gasolina")))) {
						borrar_index = true;
						break;
					} else {
						borrar_index = false;
	...
	borrarMenu("${dataForm.bPredial}", "${dataForm.bVehicular}",
			"${dataForm.bIca}", "${dataForm.bPublicidadExt}",
			"${dataForm.bSobreGasolina}", "${dataForm.bDelineacionUrbana}");
</script>