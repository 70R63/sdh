...
<c:if test="${predialFormseis.controlCampos.btnPresentarDec == false}">
	<c:set var="flagPresentarDeclaracion" value="true" />
</c:if>
<c:if test="${predialFormseis.controlCampos.btnPagarDec == false}">
...
<c:if test="${fn:substring(predialFormseis.opcionuso,0,2) == '01' && predialFormseis.indicadorspac == 'X'}">
...
<!-- Fin de campos para calculo -->
	<input type="hidden" id="numForm" value="${predialFormseis.numFrom}"/>
			...
			<div class="col-md-12 centercol-md-8 text-center">
				...
				<c:if test="${flagSPAC eq true}">
					<button class="btn btn-primary btn-lg btnspac" type="submit" id="btnspac"
                        name="pagar" value="pagar" data-numbp="${predialFormseis.numBP}" data-numForm="${predialFormseis.numFrom}" data-anio="${predialFormseis.anioGravable}" data-obj="${predialFormseis.objetocontrato}" data-reimpresion="">
