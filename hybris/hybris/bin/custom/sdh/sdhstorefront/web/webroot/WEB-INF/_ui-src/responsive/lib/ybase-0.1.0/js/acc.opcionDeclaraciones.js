ACC.opcionDeclaraciones = {
	...
	_autoload : [ "bindDeclaracionPDF", "bindCertiPagosImprime",
			"bindDeclaracionImprime" ],
					...
					if (ACC.opcionDeclaraciones.validarAntesSubmit()) {
						...
						for (var i = 0; i < e.target.form.length; i++) {
							...
							if (valorCampo != null) {
								...
								if (seleccionado == true) {
									valNumObjeto = $.trim($(valorCampo).attr(
											"data-numObjeto"));
									valNumRadicado = $.trim($(valorCampo).attr(
											"data-numRadicado"));
									break;
								}
							} else {
						...
						$.ajax({
							...
							success : function(dataResponse) {
								ACC.opcionDeclaraciones.updateFromResponsePDF(
				...
				function(e) {
					...
					if (ACC.opcionDeclaraciones.validarAntesSubmit()) {
						...
						for (var i = 0; i < e.target.form.length; i++) {
							...
							if (valorCampo != null) {
								...
								if (seleccionado == true) {
									valNumObjeto = $.trim($(valorCampo).attr(
											"data-numObjeto"));
									valCtaContrato = $.trim($(valorCampo).attr(
											"data-ctaContrato"));
									valClavePeriodo = $.trim($(valorCampo)
											.attr("data-clavePeriodo"));
									valReferencia = $.trim($(valorCampo).attr(
											"data-referencia"));
									valImporte = $.trim($(valorCampo).attr(
											"data-importe"));
									valFechaCompensa = $.trim($(valorCampo)
											.attr("data-fechaCompensa"));
									valMoneda = $.trim($(valorCampo).attr(
											"data-moneda"));
									valNumDocPago = $.trim($(valorCampo).attr(
											"data-numDocPago"));
						...
						$.ajax({
							...
							success : function(dataResponse) {
								ACC.opcionDeclaraciones
										.updateFromResponseImprimir(dataActual,
												dataResponse);
							},
							error : function() {
								alert("Error procesar la solicitud");
	...
	obtenerListaDeclaraciones : function() {
		...
		if (ACC.opcionDeclaraciones.validarAntesSubmit()) {
			...
			dataActual.claveImpuesto = claveImpuesto;
			dataActual.anoGravable = anoGravable;
			dataActual.periodo = ACC.opcionDeclaraciones
					.obtenerPeriodoPorImpuesto(claveImpuesto);
			...
			$.ajax({
				...
				type : "GET",
				success : function(dataResponse) {
					ACC.opcionDeclaraciones.updateFromResponseSeleccion(
							dataActual, dataResponse);
				},
				error : function() {
					alert("Error procesar la solicitud");
		...
		if (ACC.opcionDeclaraciones.validarAntesSubmit()) {
			...
			$.ajax({
				...
				success : function(dataResponse) {
					...
					ACC.opcionDeclaraciones
							.mostrarErrores_certiPagos(dataResponse);
					ACC.opcionDeclaraciones
							.updateFromResponseSeleccion_certiPagos(dataActual,
									dataResponse, null);
					ACC.opcionDeclaraciones.habilitarFiltroPeriodo(dataActual,
							dataResponse);
				},
				error : function() {
					alert("Error procesar la solicitud");
		...
		if (ACC.opcionDeclaraciones.validarAntesSubmitPeriodo()) {
			...
			$
					.ajax({
						...
						type : "GET",
						success : function(dataResponse) {
							ACC.opcionDeclaraciones.updateFromResponsePeriodo(
									dataActual, dataResponse);
						},
						error : function() {
	...
	mostrarErrores_certiPagos : function(infoResponse) {
		...
		if (infoResponse.errores != null && infoResponse.errores[0] != null
				&& infoResponse.errores[0].idmsj != 0) {
										...
										$('#table-vehicular1')
												.append(
														"<tr>"
																...
																+ value.marca
																+ '</td>'
																+ '<td><input id="registroNum_'
																+ index
																+ '" style="visibility: visible !important; margin: 0; min-height: 0;" name="action" type="radio" value="" data-numObjeto="'
																+ value.numObjeto
																+ '"' + ">"
																+ "</td>"
																+ "</tr>");
				...
				$('#table-ica1')
						.append(
								"<tr>"
										...
										+ '</td>'
										+ '<td>'
										+ infoResponse.anoGravable
										+ '</td>'
										+ '<td><input id="registroNum_'
										+ 0
										+ '" style="visibility: visible !important; margin: 0; min-height: 0;" name="action" type="radio" value="" data-numObjeto="'
										+ infoResponse.ica.numObjeto + '"'
										+ ">" + "</td>" + "</tr>");
			}
...
if(infoResponse.reteIca.consecutivo == null){
				...
				$('#table-reteica1')
						.append(
								"<tr>"
										...
										+ 0
										+ '" style="visibility: visible !important; margin: 0; min-height: 0;" name="action" type="radio" value="" data-numObjeto="'
										+ infoResponse.reteIca.numObjeto + '"'
										+ ">" + "</td>" + "</tr>");
			}
		...
		if (infoResponse.declaracionesCertiPagos.declaraciones != null) {
			if (infoResponse.declaracionesCertiPagos.declaraciones.length > 0) {
				...
				if (infoActual.claveImpuesto == '0002') {
					$
							.each(
									infoResponse.declaracionesCertiPagos.declaraciones,
									function(index, value1) {
										if (value1.numObjeto != "") {
											$
													.each(
															infoResponse.customerData.vehicular,
															function(index,
																	value2) {
																if (value1.numObjeto == value2.numObjeto) {
																	desc_clavePeriodo = ACC.opcionDeclaraciones
																			.obtener_desc_clavePeriodo(value1.clavePeriodo);
																	$(
																			'#table-vehicular1')
																			.append(
																					"<tr>"
										...
										if (value1.numObjeto != "") {
											if (value1.numObjeto == infoResponse.customerData.reteIca.numObjeto) {
												$('#table-reteica1')
														.append(
																"<tr>" + '<td>'
																		+ infoResponse.customerData.reteIca.numID
																		+ '</td>'
																		+ '<td>'
																		+ infoResponse.customerData.reteIca.consecutivo
																		+ '</td>'
																		+ '<td>'
																		+ value1.clavePeriodo
																		+ '</td>'
							...
							.each(
									...
									function(index, value1) {
										if (value1.numObjeto != "") {
											$
													.each(
															infoResponse.customerData.gasolina,
															function(index,
																	value2) {
																if (value1.numObjeto == value2.numObjeto) {
																	$(
							...
							.each(
									...
									function(index, value1) {
										if (value1.numObjeto != "") {
											$
													.each(
															infoResponse.customerData.delineacion,
															function(index,
																	value2) {
																if (value1.numObjeto == value2.numObjeto) {
																	$(
							...
							.each(
									...
									function(index, value1) {
										if (value1.numObjeto != "") {
											$
													.each(
															infoResponse.customerData.publicidadExt,
															function(index,
																	value2) {
																if (value1.numObjeto == value2.numObjeto) {
																	$(
	...
	updateFromResponseSeleccion_presentarDec : function(infoActual,
			infoResponse) {
		debugger;
						...
						$
								.each(
										...
										function(index, value1) {
											...
											if (value1.numObjeto != "") {
												if (value1.numObjeto == infoResponse.customerData.ica.numObjeto) {
													tipoPeriodo = ACC.opcionDeclaraciones
															.obtener_tipoPeriodo(value1.clavePeriodo);
													if (tipoPeriodo == 'B') {
														perBimestral.style.display = 'block';
													}
	...
	validarAntesSubmit : function() {
		...
		if (claveImpuesto == "0001" || claveImpuesto == "0002"
				|| claveImpuesto == "0006" || claveImpuesto == "0007") {
		...
		if (claveImpuesto == "0005") {
			if (anoGravable != "" && anoGravable != "00" && periodoM != ""
					&& periodoM != "00") {
				validacionOK = true;
			}
		}
	...
	validarAntesSubmitPeriodo : function() {
		...
		if (claveImpuesto == "0003" || claveImpuesto == "0004"
				|| claveImpuesto == "0005") {
			validacionOK = true;
		}
	...
	obtener_desc_clavePeriodo : function(clavePeriodo) {
		...
		tipo_periodo = ACC.opcionDeclaraciones
				.obtener_tipoPeriodo(clavePeriodo);
		if (tipo_periodo == "B") {
			id_periodo = ACC.opcionDeclaraciones
					.obtener_valorPeriodo(clavePeriodo);
	...
	validarFiltro : function(itemInfo, idPeriodoFiltro) {
		...
		if (idPeriodoFiltro != null) {
			tipo_periodo = ACC.opcionDeclaraciones
					.obtener_tipoPeriodo(itemInfo.clavePeriodo);
			if (tipo_periodo == "B") {
				idPeriodoItem = ACC.opcionDeclaraciones
						.obtener_valorPeriodo(itemInfo.clavePeriodo);
	...
	obtenerListaDeclaraciones_presentarDec : function(claveImpuesto) {
		...
		if (ACC.opcionDeclaraciones.validarAntesSubmit_presentarDec()) {
			...
			$.ajax({
				...
				success : function(dataResponse) {
					ACC.opcionDeclaraciones
							.updateFromResponseSeleccion_presentarDec(
									dataActual, dataResponse);
				},
				error : function() {
					alert("Error procesar la solicitud");
