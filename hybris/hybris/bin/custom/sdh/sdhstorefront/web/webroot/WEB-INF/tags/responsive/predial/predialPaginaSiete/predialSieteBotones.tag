...
<c:if test="${predialFormsiete.controlCampos.btnPresentarDec == false}">
	<c:set var="flagPresentarDeclaracion" value="true" />
</c:if>
<c:if test="${predialFormsiete.controlCampos.btnPagarDec == false}">
...
<c:if test="${fn:substring(predialFormsiete.opcionuso,0,2) == '01' && predialFormsiete.indicadorspac == 'X'}">
...
<!-- Fin de campos para calculo -->
	<input type="hidden" id="numForm" value="${predialFormsiete.numFrom}"/>
			...
			<div class="col-md-12 centercol-md-8 text-center">
				...
				<c:if test="${flagSPAC eq true}">
					<button class="btn btn-primary btn-lg btnspac" type="submit" id="btnspac"
                        name="pagar" value="pagar" data-numbp="${predialFormsiete.numBP}" data-numForm="${predialFormsiete.numFrom}" data-anio="${predialFormsiete.anioGravable}" data-obj="${predialFormsiete.objetocontrato}" data-reimpresion="">
