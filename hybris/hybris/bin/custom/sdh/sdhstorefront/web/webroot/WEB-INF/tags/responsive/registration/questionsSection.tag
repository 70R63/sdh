					...
					<fieldset>
						...
						<c:forEach  items="${questionAndOptions}" var="entry">
							<c:set var = "keyQuestionDesc" value = "${entry.key}"/>
 							<formElement:formSelectBox idKey="${entry.key}" labelKey="${questionCatalog[keyQuestionDesc]}" path="${entry.key}" mandatory="true" skipBlank="false" skipBlankMessageKey="address.title.pleaseSelect"  items="${entry.value}" selectedValue="" selectCSSClass="form-control"/>
						</c:forEach>			
