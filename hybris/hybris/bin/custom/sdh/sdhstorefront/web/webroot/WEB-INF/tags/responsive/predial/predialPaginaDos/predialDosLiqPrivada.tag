...
<c:choose>
<c:when test="${predialFormdos.checkAporte_flag == 1}">
	<c:set var="disabledAporte" value='' />
</c:when>
<c:when test="${predialFormdos.checkAporte_flag == 2}">
...
<c:if test="${predialFormdos.anioGravable != '2022'}">
...
<form:form>
	<div class="container">
		...
		<div class="row">
			<c:if test="${predialFormdos.aporte_activo == 'X'}">
