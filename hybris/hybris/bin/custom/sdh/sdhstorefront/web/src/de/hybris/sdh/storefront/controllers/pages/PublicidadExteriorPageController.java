...
public class PublicidadExteriorPageController extends AbstractPageController
{
	...
	public String showAll(final Model model, final HttpServletRequest request) throws CMSItemNotFoundException
	{
		final CustomerModel customerModel = (CustomerModel) userService.getCurrentUser();
		final ConsultaContribuyenteBPRequest consultaContribuyenteBPRequest = new ConsultaContribuyenteBPRequest();
		final PublicidadForm publicidadForm = new PublicidadForm();
		...
		String anoGravable = request.getParameter("anoGravable");

		if (StringUtils.isBlank(anoGravable))
		{

			anoGravable = String.valueOf(LocalDate.now().getYear());

		}

		model.addAttribute("selectedYear", anoGravable);
		...
		try
		{
					...
					&& !sdhConsultaContribuyenteBPResponse.getPublicidadExt().isEmpty())
			{
				...
				final Map<String, List<PublicidadForm>> allDetailsMap = new TreeMap<String, List<PublicidadForm>>();

				for(final ImpuestoPublicidadExterior eachPublicicidad :sdhConsultaContribuyenteBPResponse.getPublicidadExt())
				{

					if (StringUtils.isBlank(eachPublicicidad.getNumResolu()))
					{
						continue;
					}

					final PublicidadForm eachDetallePublicidad = this.getDetails(numBP, eachPublicicidad.getNumResolu(), anoGravable,
							eachPublicicidad.getTipoValla());

					if (allDetailsMap.containsKey(eachPublicicidad.getTipoValla()))
					{
						allDetailsMap.get(eachPublicicidad.getTipoValla()).add(eachDetallePublicidad);
					}
					else
					{
						final List<PublicidadForm> temp = new ArrayList<PublicidadForm>();

						temp.add(eachDetallePublicidad);

						allDetailsMap.put(eachPublicicidad.getTipoValla(), temp);

					}

				}

				model.addAttribute("allDetailsMap", allDetailsMap);
	...
	public PublicidadForm getDetails(final String numBP, final String numResolu, final String anoGravanle, final String tipoValla)
			throws CMSItemNotFoundException
	{
		...
		detallePublicidadRequest.setAnoGravable(anoGravanle);
		...
		try
		{
			...
			publicidadForm.setNumResolu(detallePublicidadResponse.getNumResolu());
			...
			if (detallePublicidadResponse.getDetalle() != null && !detallePublicidadResponse.getDetalle().isEmpty())
			{
				...
				for (final DetallePubli eachDetalle : detallePublicidadResponse.getDetalle())
				{
					...
					else if ("Valla Tubular de Obra".equalsIgnoreCase(tipoValla))
					{
						...
						if ("01".equals(detallePublicidadResponse.getTipoSolicitud())
								|| "01".equals(detallePublicidadResponse.getTipoSolicitud()))
						{
							publicidadForm.setTipoSolicitud("Registro Nuevo");
						}
						else if ("02".equals(detallePublicidadResponse.getTipoSolicitud())
								|| "02".equals(detallePublicidadResponse.getTipoSolicitud()))
						{
							publicidadForm.setTipoSolicitud("Actualizacion");
						}
						else if ("03".equals(detallePublicidadResponse.getTipoSolicitud())
								|| "03".equals(detallePublicidadResponse.getTipoSolicitud()))
						{
							publicidadForm.setTipoSolicitud("Prorroga");
						}
						else if ("04".equals(detallePublicidadResponse.getTipoSolicitud())
								|| "04".equals(detallePublicidadResponse.getTipoSolicitud()))
						{
							publicidadForm.setTipoSolicitud("Traslado");
						}
						else
