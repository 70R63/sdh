...
public abstract class SDHAbstractRegisterPageController extends AbstractPageController
{
	protected static final String CHECKOUT_ORDER_CONFIRMATION_CMS_PAGE_LABEL = "orderConfirmation";

	private static final Logger LOGGER = Logger.getLogger(SDHAbstractRegisterPageController.class);
	...
	protected String getDefaultRegistrationPage(final Model model) throws CMSItemNotFoundException
	{
		...
		final String emailFromSAP = getSessionService().getAttribute("SMTP_ADDR");

		final SDHRegisterForm registerForm = new SDHRegisterForm();
		if (emailFromSAP != null)
		{
			registerForm.setEmail(emailFromSAP);
			registerForm.setConfirmEmail(emailFromSAP);
		}

		model.addAttribute("SDHRegisterForm", registerForm);
		return getView();
	}
	 ...
	 */
	protected String processRegisterUserRequest(final String referer, final SDHRegisterForm form,
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException // NOSONAR
	{
		...
		data.setUseEmailForNotifications(form.getUseEmailForNotifications());
