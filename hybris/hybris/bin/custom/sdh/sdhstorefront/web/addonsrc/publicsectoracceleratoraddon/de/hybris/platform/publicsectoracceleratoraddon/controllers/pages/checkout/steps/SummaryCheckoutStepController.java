...
import de.hybris.platform.publicsectoracceleratoraddon.forms.PlaceOrderForm;
...
public class SummaryCheckoutStepController extends AbstractCheckoutStepController
{
	...
	protected static final String EMBEDDED_FORM_HTMLS = "embeddedFormHtmlsReview";
			...
			throws CMSItemNotFoundException
	{
		...
		storeServiceRelatedDataInModel(model, cartData);
			...
			throws CMSItemNotFoundException
	{
		...
		if (psCheckoutFacade.isPaymentNeeded())
		{
	...
	/**
	 * fills the model with data related with service
	 *
	 * @param model
	 * @param cartData
	 */
	protected void storeServiceRelatedDataInModel(final Model model, final CartData cartData)
	{
		if (!CollectionUtils.isEmpty(cartData.getEntries()))
		{
			final OrderEntryData orderEntry = getPsCheckoutFacade().getServiceProductOrderEntryFromCart();
			final ProductData product = orderEntry.getProduct();
			model.addAttribute("consentDeclarations", product.getConsentDeclarations());
			model.addAttribute("productAddToCartText", product.getAddToCartLabelText());
			model.addAttribute(EMBEDDED_FORM_HTMLS, getPsYFormTemplateFacade().getYFormHTMLForService(cartData));
			model.addAttribute("bundleInfo", getBundleSelectionFacade().getSelectedBundlesInformationForCart(cartData));
		}
	}
	...
	protected boolean validateOrderForm(final PlaceOrderForm placeOrderForm, final Model model)
	{
		...
		if (validateConsentDeclarations(placeOrderForm, model, cartData))
		{
			invalid = true;
		}
	 ...
	 * @param cartData
	 */
	private boolean validateConsentDeclarations(final PlaceOrderForm placeOrderForm, final Model model, final CartData cartData)
	{
		final List<OrderEntryData> cartEntries = cartData.getEntries();
		if (CollectionUtils.isNotEmpty(cartEntries))
		{
			final ProductData product = cartEntries.get(0).getProduct();

			if (CollectionUtils.isNotEmpty(product.getConsentDeclarations()) && (placeOrderForm.getConsentDeclarations() == null
					|| placeOrderForm.getConsentDeclarations().length != product.getConsentDeclarations().size()))
			{
