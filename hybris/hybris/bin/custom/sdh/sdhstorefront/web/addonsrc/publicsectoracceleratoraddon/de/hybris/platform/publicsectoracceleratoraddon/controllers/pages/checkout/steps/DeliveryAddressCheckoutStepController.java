...
import de.hybris.platform.publicsectoracceleratoraddon.forms.AddressForm;
...
{
	private static final String REDIRECT_CHECKOUT_MULTI = "redirect:/checkout/multi/delivery-address/save";
	...
	private static final String CHECKOUT_ADDRESS_UPDATED = "checkout.multi.address.updated";
	...
	public String enterStep(final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		final AddressData primaryAddress = getUserFacade().getDefaultAddress();
		...
		{
			model.addAttribute("showUsePrimaryAddress", Boolean.TRUE);
			model.addAttribute("primaryAddress", primaryAddress);
		}

		if (deliveryAddress != null)
		{
			model.addAttribute("showSelectedDeliveryAddress", Boolean.TRUE);
			model.addAttribute("savedDeliveryAddress", deliveryAddress);
		}

		if (primaryAddress != null && deliveryAddress != null && primaryAddress.getId().equalsIgnoreCase(deliveryAddress.getId()))
		{
			model.addAttribute("showSelectedDeliveryAddress", Boolean.FALSE);
		}
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		if (BooleanUtils.isNotTrue(addressForm.getUsePrimaryAddress()))
		{
			newAddress = populateAddressData(addressForm);
			if (StringUtils.isEmpty(addressForm.getAddressId()))
			{
				...
				final boolean addressRequiresReview = getAddressVerificationResultHandler().handleResult(verificationResult,
						...
						getAddressVerificationFacade().isCustomerAllowedToIgnoreAddressSuggestions(), CHECKOUT_ADDRESS_UPDATED);
		...
		{
			newAddress = getUserFacade().getDefaultAddress();
		}
	...
	@RequireHardLogIn
	public String enterStepSave(final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
	...
	@RequireHardLogIn
	public String save(final AddressForm addressForm, final BindingResult bindingResult) throws CMSItemNotFoundException
	{
		getAddressValidator().validate(addressForm, bindingResult);
		...
		if (!bindingResult.hasErrors())
		{
			...
			if (BooleanUtils.isNotTrue(addressForm.getUsePrimaryAddress()))
			{
				...
				if (addressForm.getSaveInAddressBook() != null)
				{
					...
					if (addressForm.getSaveInAddressBook().booleanValue() && getUserFacade().getAddressBook().isEmpty())
					{
	 ...
	 * @return AddressData
	 */
	private AddressData populateAddressData(final AddressForm addressForm)
	{
			...
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		if (addressData != null)
		{
			model.addAttribute(SHOW_SAVE_TO_ADDRESS_BOOK, Boolean.valueOf(!addressData.isVisibleInAddressBook()));
		}
		return ControllerConstants.Views.Pages.MultiStepCheckout.AddEditDeliveryAddressPage;
	}
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		final AddressData newAddress = populateAddressData(addressForm);
		newAddress.setId(addressForm.getAddressId());
