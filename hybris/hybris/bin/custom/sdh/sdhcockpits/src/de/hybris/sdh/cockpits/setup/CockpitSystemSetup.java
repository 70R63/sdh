 ...
 */
package de.hybris.sdh.cockpits.setup;
...
import de.hybris.sdh.cockpits.constants.SdhCockpitsConstants;
 ...
 */
@SystemSetup(extension = SdhCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(SdhCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(SdhCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						SdhCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/sdhcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
